...
public class PlayerSelectGui {
    ...
    public static void open(BalancedTeamPlugin plugin, Player player, int page) {
        ...
        String title = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_TITLE, titleMap);
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            if (inMyTeam) {
                ...
                String name = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_ITEM_IN_MY_TEAM_NAME, itemMap);
                List<String> lore = plugin.getConfigManager().getMessageList(GuiConfigKeys.PLAYER_SELECT_ITEM_IN_MY_TEAM_LORE, itemMap);
                ...
                holder.setClickHandler(slot, e -> {
                    ...
                    MessageUtil.sendMessage(player, plugin.getConfigManager().getMessage("team_invite_target_in_your_team"));
                });
                ...
                String name = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_ITEM_ALREADY_INVITED_NAME, itemMap);
                List<String> lore = plugin.getConfigManager().getMessageList(GuiConfigKeys.PLAYER_SELECT_ITEM_ALREADY_INVITED_LORE, itemMap);
                ...
                String name = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_ITEM_IN_TEAM_NAME, itemMap);
                List<String> lore = plugin.getConfigManager().getMessageList(GuiConfigKeys.PLAYER_SELECT_ITEM_IN_TEAM_LORE, itemMap);
                ...
                // 可邀请玩家
                String name = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_ITEM_AVAILABLE_NAME, itemMap);
                List<String> lore = plugin.getConfigManager().getMessageList(GuiConfigKeys.PLAYER_SELECT_ITEM_AVAILABLE_LORE, itemMap);
                ...
                holder.setClickHandler(slot, e -> {
                    ...
                    if (!target.isOnline()) {
                        MessageUtil.sendMessage(player, plugin.getConfigManager().getMessage("target_player_not_found", itemMap));
                        SoundUtil.playError(player);
        ...
        if (targetPlayers.isEmpty()) {
            String emptyName = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_NO_PLAYERS_NAME);
            List<String> emptyLore = plugin.getConfigManager().getMessageList(GuiConfigKeys.PLAYER_SELECT_NO_PLAYERS_LORE, Collections.emptyMap());
        ...
        if (currentPage > 1) {
            ...
            String prevName = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_PREV_PAGE, prevMap);
            PagedGuiHelper.setupPrevButton(holder, inv, 45, player, currentPage, prevName, () -> open(plugin, player, currentPage - 1));
        }

        // 手动输入玩家名按钮 (槽位 48)
        String manualName = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_MANUAL_INPUT_NAME);
        List<String> manualLore = plugin.getConfigManager().getMessageList(GuiConfigKeys.PLAYER_SELECT_MANUAL_INPUT_LORE, Collections.emptyMap());
        ...
        holder.setClickHandler(48, e -> {
            ...
            String prompt = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_CHAT_PROMPT);
        ...
        // 返回按钮 (槽位 49)
        String backName = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_BACK_BUTTON);
        ...
        if (currentPage < totalPages) {
            ...
            String nextName = plugin.getConfigManager().getRawMessage(GuiConfigKeys.PLAYER_SELECT_NEXT_PAGE, nextMap);
