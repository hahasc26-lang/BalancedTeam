...
- **外交停战与求和机制 (Truce & Peace System)**：
  - 新增完整的停战与求和子命令体系（`/team truce`，别名 `/team peace`）：
    - `/team truce <队伍>`：向敌对团队发送停战求和请求；
    - `/team truce accept <队伍>`：接受目标敌对团队发来的求和请求；
    - `/team truce deny <队伍>`：拒绝敌对团队发来的求和请求；
    - `/team truce cancel <队伍>`：撤销己方已发出的求和请求；
    - `/team truce list`：列出当前队伍发起或收到的所有处于等待状态的求和请求；
  - **战后停战保护期 (Post-War Protection)**：
    - 停战协议达成后，双方队伍立即进入配置时长的战后保护期（`balance.post_war_protection_seconds`，默认 1800 秒）；
    - 保护期内双向禁止重新宣战，防止“假意求和后立即再次宣战偷袭”；
    - `DamageListener` 深度拦截保护期内两队成员间的所有近战物理伤害、弓箭/三叉戟抛射物伤害、喷溅药水以及滞留药水云伤害；
  - **求和请求自动过期 (Truce Expiration)**：
    - 新增 `balance.truce_request_timeout_seconds` 配置项（默认 1800 秒），超时未回复的停战请求将自动失效并清理；
...
- **安全防御加固**：
  - 修复 PlaceholderAPI 聊天占位符在玩家输入内容中被二次解析的注入漏洞，调整格式化求值顺序；
...
- **Diplomacy Truce & Peace System (`/team truce` / `/team peace`)**:
  ...
  - **Truce Request Timeout**: Added `balance.truce_request_timeout_seconds` (default: 1800s);
  - **GUI Integration**:
